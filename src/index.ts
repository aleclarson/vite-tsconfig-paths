  ...
  const plugin = {
    ...
    configResolved(config: Pick<vite.ResolvedConfig, 'logger' | 'root'>) {
      ...
      if ('rolldownVersion' in vite) {
        debug('Running under Rolldown-powered Vite')
      }
    ...
    resolveId: {
      // Hook filter: skip relative imports and virtual modules on the
      ...
      async handler(id: string, importer: string | undefined) {
        ...
        // These checks are still needed for runtimes that ignore the
        // hook filter (older Vite/Rollup versions).
